...
public class CifParserTest {
    ...
    public void parseBinary() throws IOException, ParsingException {
        for (Map.Entry<String, List<Object>> testCase : testCases.entrySet()) {
            ...
            checkParsedEntity(CifParser.parseBinary(inputStream), testCase.getValue());
        }
    }

    @Test
    public void parseText() throws ParsingException {
        for (Map.Entry<String, List<Object>> testCase : testCases.entrySet()) {
            ...
            checkParsedEntity(CifParser.parseText(inputStream), testCase.getValue());
        }
    }
    ...
    public void shouldReportExceptionForEmptyBinaryFile() throws ParsingException, IOException {
        CifParser.parseBinary(TestHelper.getInputStream("bcif/0emp.bcif"));
    }

    @Test(expected = ParsingException.class)
    public void shouldReportExceptionForWrongBinaryContent() throws ParsingException, IOException {
        CifParser.parseBinary(TestHelper.getInputStream("cif/1acj.cif"));
    }
    ...
    public void shouldReportExceptionForEmptyTextFile() throws ParsingException {
        CifParser.parseText(TestHelper.getInputStream("cif/0emp.cif"));
    }

    @Test(expected = ParsingException.class)
    public void shouldReportExceptionForWrongTextContent() throws ParsingException {
        CifParser.parseText(TestHelper.getInputStream("bcif/1acj.bcif"));
    }
}